...
const Register = () => {
    ...
    return (
        <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
            ...
            {status && (
                <div className={`mt-6 p-3 rounded-lg text-center text-sm font-semibold ${status.includes('Success') ? 'bg-emerald-50 text-emerald-700' : (status.includes('Processing') ? 'bg-blue-50 text-blue-700' : 'bg-rose-50 text-rose-700')}`}>
                    {status}
                </div>
            )}
        </div>
